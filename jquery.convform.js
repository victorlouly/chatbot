...
ConvState.prototype.answerWith = function(answerText, answerObject) {
    // console.log('resposta anterior: ', responderObject);
    //coloca resposta dentro do conjunto respostas para dar perguntas acesso a respostas anteriores
    ...
    // impressões respondem dentro do invólucro de mensagens
    ...
    //remove opções antes de anexar mensagem para que a animação de rolagem é executada sem problemas
    $(this.wrapper).find("div.options div.option").remove();
    ...
    this.parameters.eventList.onInputSubmit(this, function(){
        //vai para o próximo estado e imprime pergunta
        if(this.next()){
            setTimeout(function(){
                this.printQuestion();
            }.bind(this), 300);
...
(function($){
    $.fn.convform = function(options){
        ...
        var parameters = $.extend(true, {}, {
            ...
            selectInputDisabledText: 'Selecione a opção',
            eventList : {
        ...
        /*
        * isso criará uma matriz com todas as entradas, seleções e áreas de texto encontradas
        * dentro do invólucro, em ordem de aparência
        */
        ...
        if(inputs.length) {
            //esconde a forma original para que os usuários não possam interagir com ele
            ...
            //anexa invólucro de mensagens e forma recém-criada com a carga giratória
            ...
            //cria novo estado único com primeira entrada
            var singleState = new SingleConvState(inputs[0]);
            //cria novo estado de invólucro com primeiro estado único como atual e dá acesso ao elemento wrapper
            var state = new ConvState(wrapper, singleState, form, parameters, originalFormHtml);
            //cria todos os novos estados únicos com entradas em ordem
            for(var i in inputs) {
            ...
            //vincula entrar para responder enviar e alterar evento para procurar selecionar possíveis respostas
            ...
            //vincula formulário submeter-se a funções estaduais
